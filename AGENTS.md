...
hackathon-craft-station/
├── apps/
│   └── web/                   # Next.js 15 frontend application & dashboard
├── packages/
│   ├── backend/               # Convex backend (schema, functions, HTTP ingest)
│   ├── shared-types/          # Zod schemas & shared TypeScript interfaces
│   ├── sanitizer/             # Zero-leak secret, PII & path redaction engine
│   ├── collector/             # Event queue, Claude/Codex normalizers, batch sender
│   ├── value-engine/          # Deterministic pre-filter & educational value scoring
│   ├── image-renderer/        # 1080x1350 programmatic Social Post card component
│   ├── env/                   # Type-safe environment variables
│   └── config/                # Shared Tailwind & TypeScript configs
└── plugins/
    ├── claude-code/           # Claude Code CLI hook integration
    └── codex/                 # Codex agent hook integration
```
